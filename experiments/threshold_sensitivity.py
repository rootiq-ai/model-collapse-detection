...
The lead time should hold across a range of detection thresholds. This script
does NOT re-run training; it post-processes the metric-vs-generation series
already produced by run_experiment.py and reports, for each threshold T in
{5%, 10%, 15%, 20%}, the generation at which Distinct-1 first drops by T and
perplexity first rises by T.
...
def main():
    ...
    for name, panels in load_one_file(args.input).items():
        ...
        print("\nReading: if 'Lead' is consistently positive across thresholds,")
